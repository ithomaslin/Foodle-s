...
@interface FifthViewController () <PopupDelegate>

@end

@implementation FifthViewController

@synthesize fishTableData;
@synthesize fishTableDetail;
@synthesize fishTableImage;
@synthesize fishTableThumb;
@synthesize nutrition;

@synthesize menuButton;
@synthesize orderListButton;
@synthesize fishViewNavigationBar;
@synthesize fishViewNavigationBarTitle;
@synthesize fifthTableView;
...
- (void)search
{
    ...
    NSPredicate *initialPredicate = [NSPredicate predicateWithFormat:@"dishType == %@", @"seafood"];
    ...
    else
    {
        ...
        fishTableData = [[NSArray alloc] initWithArray:titleArray];
        fishTableDetail = [[NSArray alloc] initWithArray:detailArray];
        fishTableImage = [[NSArray alloc] initWithArray:imageArray];
        fishTableThumb = [[NSArray alloc] initWithArray:thumbArray];
    }
}
...
- (void)viewDidLoad
{
    ...
    [self.fishViewNavigationBar setBackgroundImage:[UIImage imageNamed:@"nav_bar.png"] forBarMetrics:UIBarMetricsDefault];
    self.fishViewNavigationBarTitle.title = @"SEAFOOD";
    ...
    bgImgView.frame = fifthTableView.bounds;
    fifthTableView.backgroundView = bgImgView;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIFont *sketchslab = [UIFont fontWithName:@"Sketchslab" size:30.0f];
    ...
    cell.textLabel.text = [fishTableData objectAtIndex:indexPath.row];
    cell.detailTextLabel.text = [fishTableDetail objectAtIndex:indexPath.row];
    cell.imageView.image = [UIImage imageNamed:[fishTableThumb objectAtIndex:indexPath.row]];
    ...
    if ([cell.textLabel.text isEqualToString:fishTableData[0]]) {
        cell.imageView.image = [UIImage imageNamed:@"thumb-shrimp-jumbo.png"];
    } else if ([cell.textLabel.text isEqualToString:fishTableData[1]]) {
        cell.imageView.image = [UIImage imageNamed:@"thumb-shrimp-peppercorn.png"];
    } else if ([cell.textLabel.text isEqualToString:fishTableData[2]]) {
        cell.imageView.image = [UIImage imageNamed:@"thumb-shrimp-honeywalnut.png"];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    popupViewController.popupTitle.text = [self.fishTableData objectAtIndex:indexPath.row];
    ...
    switch (indexPath.row) {
        case 0: {
            popupViewController.popupImage.image = [UIImage imageNamed:fishTableImage[indexPath.row]];
            popupViewController.popupDetail.text = fishTableDetail[indexPath.row];
